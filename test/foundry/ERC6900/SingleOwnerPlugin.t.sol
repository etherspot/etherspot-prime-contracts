...
import {ISingleOwnerPlugin} from "../../../src/ERC6900/erc6900-ref-impl/plugins/owner/ISingleOwnerPlugin.sol";
import {ContractOwner} from "../../../src/ERC6900/erc6900-ref-impl/test/mocks/ContractOwner.sol";
...
contract SingleOwnerPluginTest is Test {
    ...
    function test_requireOwner() public {
        ...
        plugin.runtimeValidationFunction(
            uint8(
                ISingleOwnerPlugin.FunctionId.RUNTIME_VALIDATION_OWNER_OR_SELF
            ),
            owner1,
            0,
            ""
    ...
    function testFuzz_isValidSignatureForEOAOwner(
        string memory salt,
        bytes32 digest
    ) public {
        // range bound the possible set of priv keys
        ...
        // sig check should fail
        assertEq(
            plugin.isValidSignature(digest, abi.encodePacked(r, s, v)),
            bytes4(0xFFFFFFFF)
        );
