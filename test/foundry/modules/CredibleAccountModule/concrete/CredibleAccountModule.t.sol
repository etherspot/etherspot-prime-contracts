...
import "../../../../../src/modular-etherspot-wallet/modules/validators/CredibleAccountModule.sol";
...
contract CredibleAccountModule_Concrete_Test is LocalTestUtils {
    ...
    // Test: Verify that the CredibleAccountModule module can be uninstalled
    function test_uninstallModule() public {
        ...
        assertEq(
            credibleAccountModule.getAssociatedSessionKeys().length,
            1,
        ...
        assertFalse(
            mew.isModuleInstalled(
                1,
                address(credibleAccountModule),
                "CredibleAccountModule module should not be installed"
            )
        );
        assertFalse(
            credibleAccountModule.isInitialized(address(mew)),
            "CredibleAccountModule module should not be initialized"
        );
        assertEq(
            credibleAccountModule.getAssociatedSessionKeys().length,
            0,
    ...
    function test_fail_uninstallModule_withActiveUnclaimedSessions() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                CredibleAccountModule
                    .CredibleAccountModule_LockedTokensNotClaimed
                    .selector,
                sessionKey
            )
        );
    ...
    function test_successfulDisableSessionKey() public {
        ...
        ICAM.SessionData memory sessionData = credibleAccountModule
    ...
    function test_fail_DisableNonExistingSessionKey() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                CredibleAccountModule
                    .CredibleAccountModule_SessionKeyDoesNotExist
                    .selector,
                dummySessionKey
            )
        );
    ...
    function test_validateSessionKeyParams() public {
        ...
        userOps[0] = userOp;
        assertTrue(
            credibleAccountModule.validateSessionKeyParams(sessionKey, userOp),
    ...
    function test_validateUserOp() public {
        ...
        (
            ,
            ICAM.SessionData memory sessionDataStruct
        ) = _enableSessionKeyAndValidate(
                credibleAccountModule,
                mew,
                IERC20.transferFrom.selector
        ...
        uint256 validationData = credibleAccountModule.validateUserOp(
            userOp,
            hash
        );
    ...
    function test_successfulSolverPartialClaimIsNotClaimed() public {
        ...
        (uint256 usdcLocked, uint256 usdcClaimed) = credibleAccountModule
            .getTokenAmounts(sessionKey, tokens[0]);
        (uint256 daiLocked, uint256 daiClaimed) = credibleAccountModule
            .getTokenAmounts(sessionKey, tokens[1]);
        (uint256 uniLocked, uint256 uniClaimed) = credibleAccountModule
    ...
    function test_exposed_validateTokenData() public {
        ...
        ICAM.LockedToken[] memory lockedTokens = new ICAM.LockedToken[](3);
        lockedTokens[0] = ICAM.LockedToken({
            token: tokens[0],
            lockedAmount: amounts[0],
            claimedAmount: 0
        });
        lockedTokens[1] = ICAM.LockedToken({
            token: tokens[1],
            lockedAmount: amounts[1],
            claimedAmount: 0
        });
        lockedTokens[2] = ICAM.LockedToken({
            token: tokens[2],
    ...
    function test_validateSingleCall_fail_if_unlockAmount_exceedsLockedAmount()
        public
    {
        // Set up the test environment and enable a session key
        _testSetup();
    ...
    function test_validate_fail_unlock_makes_ClaimedAmount_exceed_LockedAmount()
        public
    {
        // Set up the test environment and enable a session key
        _testSetup();
