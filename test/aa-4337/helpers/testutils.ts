...
import { Create2Factory } from '../../../src/aa-4337/samples/Create2Factory';
...
export async function createTestAggAccount(
  ethersSigner: Signer,
  accountOwner: string,
  entryPoint: string,
  ...
  accountFactory: TestAggregatedAccountFactory;
  implementation: string;
}> {
  const accountFactory =
    _factory ??
    (await new TestAggregatedAccountFactory__factory(ethersSigner).deploy(
      accountOwner
    ));
  const implementation = await accountFactory.accountImplementation();
  await accountFactory.createAccount(entryPoint, accountOwner, 0);
  ...
  const proxy = TestAggregatedAccount__factory.connect(
    accountAddress,
    ethersSigner
  );
  return {
