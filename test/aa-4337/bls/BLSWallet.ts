...
import { DefaultsForUserOp, fillUserOp } from '../user_ops/UserOp';
...
describe('bls account', function () {
  ...
  before(async () => {
    ...
    registry = await new PersonalAccountRegistry__factory(etherSigner).deploy();
    const BLSOpenLib = await new BLSOpen__factory(
      ethers.provider.getSigner()
    ).deploy();
    blsAgg = await new BLSSignatureAggregator__factory(
      {
        'src/aa-4337/bls/lib/BLSOpen.sol:BLSOpen': BLSOpenLib.address,
      },
      ethers.provider.getSigner()
    ).deploy();
    ...
    await account1['initialize(IEntryPoint, address, uint256[4])'](
      entrypoint,
      registry.address,
      signer1.pubkey
    );
    ...
    await account2['initialize(IEntryPoint, address, uint256[4])'](
      entrypoint,
      registry.address,
      signer2.pubkey
    );
