...
import {AccountStorage, getAccountStorage, getPermittedCallKey, toFunctionReferenceArray} from "../libraries/AccountStorage.sol";
...
abstract contract BaseModularAccountLoupe is IPluginLoupe {
    ...
    function getExecutionFunctionConfig(
        bytes4 selector
    ) external view returns (ExecutionFunctionConfig memory config) {
        ...
        config.userOpValidationFunction = _storage
            .selectorData[selector]
            .userOpValidation;

        config.runtimeValidationFunction = _storage
            .selectorData[selector]
            .runtimeValidation;
    }
    ...
    function getExecutionHooks(
        bytes4 selector
    ) external view returns (ExecutionHooks[] memory execHooks) {
        AccountStorage storage _storage = getAccountStorage();

        FunctionReference[] memory preExecHooks = toFunctionReferenceArray(
            _storage.selectorData[selector].preExecHooks
        );
        ...
        for (uint256 i = 0; i < numHooks; ) {
            ...
            execHooks[i].postExecHook = _storage
                .selectorData[selector]
                .associatedPostExecHooks[preExecHooks[i]];
    ...
    function getPermittedCallHooks(
        address callingPlugin,
        bytes4 selector
    ) external view returns (ExecutionHooks[] memory execHooks) {
        AccountStorage storage _storage = getAccountStorage();
        ...
        FunctionReference[]
            memory prePermittedCallHooks = toFunctionReferenceArray(
                _storage.permittedCalls[key].prePermittedCallHooks
            );
        ...
        for (uint256 i = 0; i < numHooks; ) {
            ...
            execHooks[i].postExecHook = _storage
                .permittedCalls[key]
                .associatedPostPermittedCallHooks[prePermittedCallHooks[i]];
    ...
    function getPreUserOpValidationHooks(
        bytes4 selector
    ) external view returns (FunctionReference[] memory preValidationHooks) {
        preValidationHooks = toFunctionReferenceArray(
    ...
    function getPreRuntimeValidationHooks(
        bytes4 selector
    ) external view returns (FunctionReference[] memory preValidationHooks) {
        preValidationHooks = toFunctionReferenceArray(
    ...
    function getInstalledPlugins()
        external
        view
        returns (address[] memory pluginAddresses)
    {
