...
function _coalescePreValidation(
    ...
    resValidationData = (
        (validUntil1 > validUntil2)
            ? uint256(validUntil2) << 160
            : uint256(validUntil1) << 160
    );
    ...
    resValidationData |= (
        (validAfter1 < validAfter2)
            ? uint256(validAfter2) << 208
            : uint256(validAfter1) << 208
    );
...
function _coalesceValidation(
    ...
    resValidationData |= uint160(preValidationData) == 1
        ? 1
