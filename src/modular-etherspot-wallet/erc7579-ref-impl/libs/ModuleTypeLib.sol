...
library ModuleTypeLib {
    function isType(
        EncodedModuleTypes self,
        ModuleType moduleType
    ) internal pure returns (bool) {
        return
            (EncodedModuleTypes.unwrap(self) &
                (2 ** ModuleType.unwrap(moduleType))) != 0;
    }

    function bitEncode(
        ModuleType[] memory moduleTypes
    ) internal pure returns (EncodedModuleTypes) {
        uint256 result;
