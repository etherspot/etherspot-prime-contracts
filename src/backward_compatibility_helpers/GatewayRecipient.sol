...
contract GatewayRecipient {
     ...
     */
    function _initializeGatewayRecipient(address gateway_) internal {
        gateway = gateway_;
    }
     ...
     */
    function _getContextAccount() internal view returns (address) {
     ...
     */
    function _getContextSender() internal view returns (address) {
     ...
     * @return context data
     */
    function _getContextData() internal view returns (bytes calldata) {
    ...
    function _getContextAddress(uint256 offset) private view returns (address) {
        address result = address(0);

        if (_isGatewaySender()) {
            uint256 from = msg.data.length - offset;
    ...
    function _isGatewaySender() private view returns (bool) {
        bool result;
