 ...
 */
contract PersonalAccountRegistry is
    AccountRegistry,
    AccountController,
    Initializable,
    GatewayRecipient
{
     ...
     */
    function initialize(address accountImplementation_, address gateway_)
        external
        initializer
    {
        // AccountController
     ...
     * @param accountImplementation_ account implementation address
     */
    function upgrade(address accountImplementation_) external {
     ...
     * @param account account address
     */
    function deployAccount(address account) external {
    ...
    function removeAccountOwner(address account, address owner) external {
        ...
        require(owner != sender, "PersonalAccountRegistry: cannot remove self");
    ...
    function refundAccountCall(
        ...
        emit AccountCallRefunded(account, tx.origin, token, value);
     ...
     */
    function computeAccountAddress(address saltOwner)
        external
        view
        returns (address)
    {
     ...
     */
    function verifyAccountOwner(address account, address owner)
        external
        view
        returns (bool)
    {
    ...
    function isValidAccountSignature(
        ...
        return
            _verifyAccountOwner(account, messageHash.recoverAddress(signature));
    }
    ...
    function _verifySender(address account) private returns (address) {
    ...
    function _deployAccount(address account) internal {
        if (!accounts[account].deployed) {
            _deployAccount(accounts[account].salt, true);
    ...
    function _computeAccountAddress(address saltOwner)
        private
        view
        returns (address)
    {
    ...
    function _verifyAccountOwner(address account, address owner)
        private
        view
        returns (bool)
    {
