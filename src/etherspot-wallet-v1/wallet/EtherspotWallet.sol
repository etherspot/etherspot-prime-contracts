    ...
    IEtherspotWallet
{
     ...
     */
    function executeBatch(address[] calldata dest, uint256[] calldata value, bytes[] calldata func) external onlyOwnerOrEntryPoint(address(entryPoint())){
        require(dest.length == func.length && (value.length == 0 || value.length == func.length), "EtherspotWallet:: executeBatch: wrong array lengths");
        if (value.length == 0) {
    ...
    function isValidSignature(
        ...
        bytes32 ethHash = MessageHashUtils.toEthSignedMessageHash(signedMessageHash);
