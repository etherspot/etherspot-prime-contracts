...
contract BLSSignatureAggregator is IAggregator {
    ...
    function validateSignatures(
        ...
        for (uint256 i = 0; i < userOpsLen; i++) {
            ...
            messages[i] = _userOpToMessage(
                userOp,
                _getPublicKeyHash(blsPublicKeys[i])
            );
        }
        require(
    ...
    function _getUserOpHash(UserOperation memory userOp, bytes32 publicKeyHash)
        internal
        view
        returns (bytes32)
    {
    ...
    function _getPublicKeyHash(uint256[4] memory publicKey)
        internal
        pure
        returns (bytes32)
    {
        ...
        returns (bytes memory sigForUserOp)
    {
        ...
        uint256[2] memory message = _userOpToMessage(
            userOp,
            _getPublicKeyHash(pubkey)
        );
